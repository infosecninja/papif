...
def prepare_request(methods, url, match_string, timeout, default_testing_length, verbose, ignore_ssl_verification):
...
PAPIF - API endpoint fuzzer v0.1
...
def papif():
    ...
    parser.add_argument('-m', '--method', metavar='<method>', type=validate_method, help='Specify the desired request methods, accepted methods are get,post,put,delete.', nargs=1, required=True)
    ...
    default_testing_length = args.default_testing_length[0] if isinstance(args.default_testing_length,list) else args.default_testing_length
